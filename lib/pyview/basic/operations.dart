...
class CropOperation implements IImageOperation {
  ...
  late int width;
  late int height;
  CropOperation({required this.x, required this.y, required this.width, required this.height});
  @override
  PyImage execute(PyImage image) {
    PyImage? cropped = PyImage(width: width, height: height);
    for (var pixel in image) {
      if (pixel.x >= x && pixel.x < x + width && pixel.y >= y && pixel.y < y + height) {
        cropped.setPixel(pixel.x - x, pixel.y - y, img.ColorRgb8(pixel.r.toInt(), pixel.g.toInt(), pixel.b.toInt()));
      }
    }
    return cropped;
  }
...
class ZoomOperation implements IImageOperation{
  ...
  late int centerY;
  late int scale;
  ZoomOperation({required this.centerX, required this.centerY, required this.scale});
  @override
  PyImage execute(PyImage image){
    ...
    int cropX = (((centerX - image.width / 2) * scale) + image.width / 2).toInt();
    int cropY = (((centerY - image.width / 2) * scale) + image.height / 2).toInt();
    ...
    for(int y = 0; y<image.width; y++){
      for(int x = 0; x<image.width; x++){
        ...
        if(srcX >= 0 && srcX < image.width && srcY >= 0 && srcY < image.height){
          ...
          zoomed.setPixel(x, y, img.ColorRgb8(pixel.r.toInt(), pixel.g.toInt(), pixel.b.toInt()));
        }
      }
    }
    return zoomed;
  ...
  PyImage execute(PyImage image) {
    ...
    for(int y = 0; y<newHeight; y++){
      for(int x = 0; x<newWidth; x++){
        img.Pixel pixel = image.getPixel(mapping[y][x] % image.width, mapping[y][x] ~/ image.height);
        resized.setPixel(x, y, img.ColorRgb8(pixel.r.toInt(), pixel.g.toInt(), pixel.b.toInt()));
      }
    }
    return resized;
  }
  ...
  PyImage execute(PyImage image){
    ...
    for(var pixel in image){
      ...
      if(newX >= 0 && newX < image.width && newY >= 0 && newY < image.height){
        rotated.setPixel(newX, newY, img.ColorRgb8(pixel.r.toInt(), pixel.g.toInt(), pixel.b.toInt()));
      }
    }
    return rotated;
  }
