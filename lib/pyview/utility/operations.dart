...
class ConcatImage implements IImageOperation{
  ...
  PyImage execute(PyImage image){
    ...
    for (var pixel in firstImage) {
      addedImage.setPixel(pixel.x, pixel.y, img.ColorRgb8(pixel.r.toInt(), pixel.g.toInt(), pixel.b.toInt()));
    }
    for (var pixel in image) {
      addedImage.setPixel(pixel.x + firstImage.width, pixel.y, img.ColorRgb8(pixel.r.toInt(), pixel.g.toInt(), pixel.b.toInt()));
    }
    return addedImage;
  }
}
...
class AddOperation implements IImageOperation{
  ...
  PyImage execute(PyImage image){
    ...
    for (var pixel in image) {
      int r = (pixel.r.toInt() + firstImage.getPixel(pixel.x, pixel.y).r.toInt()).clamp(0, 255);
      int g = (pixel.g.toInt() + firstImage.getPixel(pixel.x, pixel.y).g.toInt()).clamp(0, 255);
      int b = (pixel.b.toInt() + firstImage.getPixel(pixel.x, pixel.y).b.toInt()).clamp(0, 255);
