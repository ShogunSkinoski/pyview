...
class SaltPepperOperation implements IImageOperation {
  ...
  SaltPepperOperation({this.whiteProbablity = 0.05, this.blackProbablity = 0.05});
  @override
  PyImage execute(PyImage image) {
    Random random = Random();
    PyImage? saltPepperImage = PyImage(width: image.width, height: image.height);
    for(int y = 0; y < image.height; y++){
      for(int x = 0; x < image.width; x++){
        if(random.nextDouble() < blackProbablity){
          ...
          saltPepperImage.setPixel(x, y, img.ColorRgb8(
  ...
  PyImage execute(PyImage image) {
    ...
    for(int y = 0; y < height; y++){
      for(int x = 0; x < width; x++){
        ...
        meanImage.setPixel(x, y, img.ColorRgb8((redSum / (kernelSize * kernelSize)).toInt(),
  ...
  PyImage execute(PyImage image){
    ...
    for(int y = 0; y < height; y++){
      for(int x = 0; x < width; x++){
        ...
        greenValues.sort();
        blueValues.sort();
        medianImage.setPixel(x, y, img.ColorRgb8(redValues[(redValues.length - 1) ~/ 2],
